...
const Projects = () => {
  ...
  const prevProjectHandler = () => {
    ...
    let newProjList = prevProjOrder.map((proj, i) => {
      if (i === prevProjOrder.length - 1) {
        return (
          <div className={[cls.MoveInToLeft, cls.DispProjContents].join(" ")} key={Math.random() * 8}>
            {proj.props.children}
          </div>
        );
      } else if (i === 0) {
        return (
          <div className={[cls.MoveLeftToCenter, cls.DispProjContents].join(" ")} key={Math.random() * 8}>
            {proj.props.children}
          </div>
        );
      } else {
        return (
          <div className={[cls.MoveCenterToRight, cls.DispProjContents].join(" ")} key={Math.random() * 8}>
            {proj.props.children}
          </div>
        );
      }
      ...
      <div className={cls.ProjectsSectionContainer}>
        <div className={cls.SliderContainer}>
          <div className={cls.ProjectDisplayContainer}>
            <div className={cls.projDispLeft}>{projectsArray[projectsArray.length - 1]}</div>
            {leftButton}
